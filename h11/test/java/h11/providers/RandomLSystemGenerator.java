...
public class RandomLSystemGenerator {
    ...
    public List<Projection> generate() {
        int randNumber = random.nextInt(1, MAX_SYSTEM_SIZE);
        return Stream.generate( () -> random.latin(1))
            .distinct()
            .limit(randNumber)
            .map(m -> makeProjection(m))
    ...
    public Projection makeProjection(String src) {
        int length = random.nextInt(1, MAX_PROJECTION_DESTINATION_SIZE);
        String des = random.latin(length);
        return new Projection(src, des);
    }
}
